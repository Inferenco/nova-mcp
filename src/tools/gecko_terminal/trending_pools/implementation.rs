...
impl TrendingPoolsTools {
    ...
    pub async fn get_trending_pools(
        ...
        let duration = input.duration.unwrap_or_else(|| "24h".to_string());
        let allowed = ["5m", "1h", "6h", "24h"];
        if !allowed.contains(&duration.as_str()) {
        ...
        let mut url = build_url(
            &self.base_url,
            &["networks", &input.network, "trending_pools"],
        );
        url.push_str(&format!(
            "?page={}&duration={}&limit={}&include=base_token,quote_token,dex",
            page, duration, limit
        ));
        let pools = self
            .http
